import { S3Client, CreateBucketCommand,
    PutObjectCommand, ListBucketsCommand,
    ListObjectsCommand, DeleteBucketCommand } from '@aws-sdk/client-s3';
...
class BucketManager { 
    ...
    deleteBucket = async () => {
        try {
            const data = await s3Client.send(
                new DeleteBucketCommand(this.params)
            );
